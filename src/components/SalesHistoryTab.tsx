...
import { Search, MessageCircle, Eye, ShoppingBag, X } from 'lucide-react';
    ...
    <div className="space-y-4">
                    ...
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center space-x-1.5">
                        <button
                          ...
                          className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs px-2.5 py-1.5 rounded-lg transition font-bold flex items-center space-x-1 cursor-pointer border border-zinc-700"
                        >
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-700 w-full max-w-lg rounded-2xl shadow-2xl p-6 space-y-4 relative max-h-[90vh] overflow-y-auto font-sans">
            <button
            ...
            <div className="border-b border-zinc-800 pb-3 space-y-1">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-zinc-400 font-mono">{formatDate(selectedOrder.date)}</span>
              </div>
              <h3 className="font-bold text-lg text-white font-tactical">
                {selectedOrder.rank} {selectedOrder.warName}
              </h3>
              <p className="text-xs text-zinc-400">
                {selectedOrder.force} {selectedOrder.battalion ? `• ${selectedOrder.battalion}` : ''} • RE: {selectedOrder.militaryId}
              </p>
            </div>
            ...
            <div className="flex justify-end space-x-2 pt-2">
              <button
                ...
                className="bg-amber-500 hover:bg-amber-400 text-black font-bold text-xs px-4 py-2 rounded-xl transition cursor-pointer"
              >
                Abrir Comprovante Imprimível
              </button>
            </div>
          </div>
        </div>
